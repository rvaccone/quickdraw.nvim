...
test("semicolon after t advances past the adjacent target", function()
	api.nvim_buf_set_lines(0, 0, -1, false, { "a x b x c" })
	api.nvim_win_set_cursor(0, { 1, 0 })
	feed("tx")
	eq(api.nvim_win_get_cursor(0), { 1, 1 }, "before the first x")
	feed(";")
	eq(api.nvim_win_get_cursor(0), { 1, 5 }, "before the second x, like native")
end)

test("macros record and replay a single application", function()
	feed("qadfzq")
	local recorded = fn.getline(1)
	feed("u")
	api.nvim_win_set_cursor(0, { 1, 0 })
	feed("@a")
	eq(fn.getline(1), recorded, "replay applies exactly once")
end)
